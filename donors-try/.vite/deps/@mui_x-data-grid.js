...
import "./chunk-SW4W6C2V.js";
import "./chunk-7VJOI2MW.js";
import {
...
import {
  Paper_default
} from "./chunk-KMNYZL2Z.js";
import {
  Box_default
} from "./chunk-STU5SKTY.js";
import "./chunk-FYZQ3HL7.js";
import {
  IconButton_default
} from "./chunk-NBJSYBQA.js";
import "./chunk-53BQFS5B.js";
import "./chunk-OPZMQGFY.js";
import {
  Button_default
} from "./chunk-KAGGYAHE.js";
import "./chunk-SOC6IBGW.js";
import "./chunk-VQYYHDLG.js";
import "./chunk-MQPAI7KU.js";
import {
  init_utils,
  useId_default
} from "./chunk-QODU2TCJ.js";
import "./chunk-6MT4EEOL.js";
import "./chunk-GKC4V64E.js";
import {
  createSvgIcon
} from "./chunk-J7UW4NGJ.js";
import "./chunk-OKPYR24P.js";
import "./chunk-TNSXN5U5.js";
import "./chunk-GVTUI223.js";
import {
...
import {
  require_react_dom
} from "./chunk-WBWDS4EP.js";
import "./chunk-REIHV5Q2.js";
...
function createUseGridApiEventHandler(registryContainer2) {
  ...
  return function useGridApiEventHandler2(apiRef, eventName, handler, options) {
    ...
    const [objectRetainedByReact] = React33.useState(new ObjectToBeRetainedByReact());
    const subscription = React33.useRef(null);
    const handlerRef = React33.useRef();
    handlerRef.current = handler;
    const cleanupTokenRef = React33.useRef(null);
...
function useGridApiMethod(privateApiRef, apiMethods, visibility) {
  const apiMethodsRef = React34.useRef(apiMethods);
  const [apiMethodsNames] = React34.useState(Object.keys(apiMethods));
  const installMethods = React34.useCallback(() => {
...
var useGridNativeEventListener = (apiRef, ref, eventName, handler, options) => {
  ...
  const [added, setAdded] = React36.useState(false);
  const handlerRef = React36.useRef(handler);
  const wrapHandler = React36.useCallback((event) => {
...
function GridColumnHeaderFilterIconButton(props) {
  ...
  const labelId = useId();
  const panelId = useId();
  const toggleFilter = React38.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    const {
      open: open2,
...
var GridCellCheckboxForwardRef = React39.forwardRef(function GridCellCheckboxRenderer(props, ref) {
  var _rootProps$slotProps;
  const {
    field,
    id,
  ...
  const checkboxElement = React39.useRef(null);
...
var _excluded32 = ["item", "applyValue", "type", "apiRef", "focusElementRef", "tabIndex", "disabled", "isFilterActive", "clearButton", "InputProps"];
...
var _excluded33 = ["item", "applyValue", "type", "apiRef", "focusElementRef", "InputProps", "isFilterActive", "clearButton", "tabIndex", "disabled"];
...
var _excluded34 = ["item", "applyValue", "type", "apiRef", "focusElementRef", "getOptionLabel", "getOptionValue", "placeholder", "tabIndex", "label", "isFilterActive", "clearButton"];
...
var _excluded35 = ["item", "applyValue", "apiRef", "focusElementRef", "isFilterActive", "clearButton", "tabIndex", "label"];
...
function GridContextProvider({
  ...
  const apiRef = React82.useRef(privateApiRef.current.getPublicApi());
...
var useGridLoggerFactory = (apiRef, props) => {
  const getLogger = React83.useCallback((name) => {
    if (forceDebug) {
...
function useGridApiInitialization(inputApiRef, props) {
  const publicApiRef = React84.useRef();
  ...
  const privateApiRef = React84.useRef();
  if (!privateApiRef.current) {
  ...
  React84.useImperativeHandle(inputApiRef, () => publicApiRef.current, [publicApiRef]);
...
var useGridLocaleText = (apiRef, props) => {
  const getLocaleText = React85.useCallback((key) => {
...
var useGridPipeProcessing = (apiRef) => {
  const processorsCache = React86.useRef({});
  const runAppliers = React86.useCallback((groupCache) => {
    if (!groupCache) {
      return;
    }
  ...
  const registerPipeProcessor = React86.useCallback((group, id, processor) => {
  ...
  const registerPipeApplier = React86.useCallback((group, id, applier) => {
    if (!processorsCache.current[group]) {
      processorsCache.current[group] = {
        processors: /* @__PURE__ */ new Map(),
        appliers: {}
  ...
  const requestPipeProcessorsApplication = React86.useCallback((group) => {
    const groupCache = processorsCache.current[group];
    runAppliers(groupCache);
  }, [runAppliers]);
  const applyPipeProcessors = React86.useCallback((...args) => {
...
var useGridStateInitialization = (apiRef, props) => {
  const controlStateMapRef = React91.useRef({});
  const [, rawForceUpdate] = React91.useState();
  const registerControlState = React91.useCallback((controlStateItem) => {
    const {
      stateId
  ...
  const setState = React91.useCallback((state, reason) => {
    let newState;
    if (isFunction(state)) {
  ...
  const updateControlState = React91.useCallback((key, state, reason) => {
  ...
  const forceUpdate = React91.useCallback(() => rawForceUpdate(() => apiRef.current.state), [apiRef]);
...
var useGridInitializeState = (initializer, privateApiRef, props) => {
  const isInitialized = React92.useRef(false);
...
var useGridClipboard = (apiRef, props) => {
  ...
  const handleCopy = React93.useCallback((event) => {
...
var useGridColumnMenu = (apiRef) => {
  ...
  const showColumnMenu = React94.useCallback((field) => {
  ...
  const hideColumnMenu = React94.useCallback(() => {
  ...
  const toggleColumnMenu = React94.useCallback((field) => {
...
function useGridColumns(apiRef, props) {
  ...
  const previousColumnsProp = React95.useRef(props.columns);
  const previousColumnTypesProp = React95.useRef(columnTypes);
  ...
  const setGridColumnsState = React95.useCallback((columnsState) => {
    logger.debug("Updating columns state.");
    apiRef.current.setState(mergeColumnsState(columnsState));
    apiRef.current.forceUpdate();
    apiRef.current.publishEvent("columnsChange", columnsState.orderedFields);
  }, [logger, apiRef]);
  const getColumn = React95.useCallback((field) => gridColumnLookupSelector(apiRef)[field], [apiRef]);
  const getAllColumns = React95.useCallback(() => gridColumnDefinitionsSelector(apiRef), [apiRef]);
  const getVisibleColumns = React95.useCallback(() => gridVisibleColumnDefinitionsSelector(apiRef), [apiRef]);
  const getColumnIndex = React95.useCallback((field, useVisibleColumns = true) => {
    const columns = useVisibleColumns ? gridVisibleColumnDefinitionsSelector(apiRef) : gridColumnDefinitionsSelector(apiRef);
    return columns.findIndex((col) => col.field === field);
  }, [apiRef]);
  const getColumnPosition = React95.useCallback((field) => {
    const index = getColumnIndex(field);
    return gridColumnPositionsSelector(apiRef)[index];
  }, [apiRef, getColumnIndex]);
  const setColumnVisibilityModel = React95.useCallback((model) => {
    const currentModel = gridColumnVisibilityModelSelector(apiRef);
  ...
  const updateColumns = React95.useCallback((columns) => {
    const columnsState = createColumnsState({
      apiRef,
      columnTypes,
  ...
  const setColumnVisibility = React95.useCallback((field, isVisible) => {
  ...
  const getColumnIndexRelativeToVisibleColumns = React95.useCallback((field) => {
    const allColumns = gridColumnFieldsSelector(apiRef);
    return allColumns.findIndex((col) => col === field);
  }, [apiRef]);
  const setColumnIndex = React95.useCallback((field, targetIndexPosition) => {
    const allColumns = gridColumnFieldsSelector(apiRef);
  ...
  const setColumnWidth = React95.useCallback((field, width) => {
  ...
  const addColumnMenuItems = React95.useCallback((columnMenuItems) => {
  ...
  const prevInnerWidth = React95.useRef(null);
  ...
  const hydrateColumns = React95.useCallback(() => {
...
var useGridDensity = (apiRef, props) => {
  ...
  const setDensity = React96.useCallback((newDensity) => {
...
var useGridCsvExport = (apiRef, props) => {
  ...
  const getDataAsCsv = React97.useCallback((options = {}) => {
  ...
  const exportDataAsCsv = React97.useCallback((options) => {
  ...
  const addExportMenuButtons = React97.useCallback((initialValue, options) => {
...
var useGridPrintExport = (apiRef, props) => {
  ...
  const doc = React98.useRef(null);
  const previousGridState = React98.useRef(null);
  const previousColumnVisibility = React98.useRef({});
  React98.useEffect(() => {
    doc.current = ownerDocument(apiRef.current.rootElementRef.current);
  }, [apiRef]);
  const updateGridColumnsForPrint = React98.useCallback((fields, allColumns) => new Promise((resolve) => {
  ...
  const handlePrintWindowLoad = React98.useCallback((printWindow, options) => {
...
`;
  ...
  const handlePrintWindowAfterPrint = React98.useCallback((printWindow) => {
  ...
  const exportDataAsPrint = React98.useCallback(async (options) => {
...
var useGridFilter = (apiRef, props) => {
  ...
  const updateFilteredRows = React99.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const addColumnMenuItem = React99.useCallback((columnMenuItems, colDef) => {
    if (colDef == null || colDef.filterable === false || props.disableColumnFilter) {
      return columnMenuItems;
    }
    return [...columnMenuItems, "columnMenuFilterItem"];
  }, [props.disableColumnFilter]);
  const applyFilters = React99.useCallback(() => {
    updateFilteredRows();
    apiRef.current.forceUpdate();
  }, [apiRef, updateFilteredRows]);
  const upsertFilterItem = React99.useCallback((item) => {
  ...
  const upsertFilterItems = React99.useCallback((items) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const deleteFilterItem = React99.useCallback((itemToDelete) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const showFilterPanel = React99.useCallback((targetColumnField, panelId, labelId) => {
  ...
  const hideFilterPanel = React99.useCallback(() => {
    logger.debug("Hiding filter panel");
    apiRef.current.hidePreferences();
  }, [apiRef, logger]);
  const setFilterLogicOperator = React99.useCallback((logicOperator) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setQuickFilterValues = React99.useCallback((values) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setFilterModel = React99.useCallback((model, reason) => {
  ...
  const flatFilteringMethod = React99.useCallback((params) => {
  ...
  const handleColumnsChange = React99.useCallback(() => {
  ...
  const handleStrategyProcessorChange = React99.useCallback((methodName) => {
    if (methodName === "filtering") {
      apiRef.current.unstable_applyFilters();
    }
  }, [apiRef]);
  const updateVisibleRowsLookupState = React99.useCallback(() => {
    apiRef.current.setState((state) => {
      return _extends({}, state, {
...
var useGridFocus = (apiRef, props) => {
  ...
  const lastClickedCell = React100.useRef(null);
  const publishCellFocusOut = React100.useCallback((cell, event) => {
    if (cell) {
  ...
  const setCellFocus = React100.useCallback((id, field) => {
    const focusedCell = gridFocusCellSelector(apiRef);
  ...
  const setColumnHeaderFocus = React100.useCallback((field, event = {}) => {
  ...
  const setColumnHeaderFilterFocus = React100.useCallback((field, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    publishCellFocusOut(cell, event);
    apiRef.current.setState((state) => {
  ...
  const setColumnGroupHeaderFocus = React100.useCallback((field, depth, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    if (cell) {
      apiRef.current.publishEvent("cellFocusOut", apiRef.current.getCellParams(cell.id, cell.field), event);
    }
  ...
  const getColumnGroupHeaderFocus = React100.useCallback(() => unstable_gridFocusColumnGroupHeaderSelector(apiRef), [apiRef]);
  const moveFocusToRelativeCell = React100.useCallback((id, field, direction) => {
  ...
  const handleCellDoubleClick = React100.useCallback(({
    id,
    field
  }) => {
    apiRef.current.setCellFocus(id, field);
  }, [apiRef]);
  const handleCellKeyDown = React100.useCallback((params, event) => {
    if (event.key === "Enter" || event.key === "Tab" || event.key === "Shift" || isNavigationKey(event.key)) {
      return;
    }
    apiRef.current.setCellFocus(params.id, params.field);
  }, [apiRef]);
  const handleColumnHeaderFocus = React100.useCallback(({
    field
  }, event) => {
  ...
  const handleColumnGroupHeaderFocus = React100.useCallback(({
    fields,
    depth
  }, event) => {
    if (event.target !== event.currentTarget) {
  ...
  const handleBlur = React100.useCallback(() => {
  ...
  const handleCellMouseDown = React100.useCallback((params) => {
    lastClickedCell.current = params;
  }, []);
  const handleDocumentClick = React100.useCallback((event) => {
  ...
  const handleCellModeChange = React100.useCallback((params) => {
  ...
  const handleRowSet = React100.useCallback(() => {
    const cell = gridFocusCellSelector(apiRef);
...
var useGridKeyboardNavigation = (apiRef, props) => {
  ...
  const theme = useTheme();
  const currentPageRows = React101.useMemo(() => enrichPageRowsWithPinnedRows(apiRef, initialCurrentPageRows), [apiRef, initialCurrentPageRows]);
  const headerFilteringEnabled = (
    // @ts-expect-error // TODO move relevant code to the `DataGridPro`
    props.signature !== "DataGrid" && props.unstable_headerFilters
  );
  const goToCell = React101.useCallback((colIndex, rowId, closestColumnToUse = "left") => {
  ...
  const goToHeader = React101.useCallback((colIndex, event) => {
  ...
  const goToHeaderFilter = React101.useCallback((colIndex, event) => {
  ...
  const goToGroupHeader = React101.useCallback((colIndex, depth, event) => {
    logger.debug(`Navigating to header col ${colIndex}`);
    apiRef.current.scrollToIndexes({
      colIndex
    });
  ...
  const getRowIdFromIndex = React101.useCallback((rowIndex) => {
    return currentPageRows == null ? void 0 : currentPageRows[rowIndex].id;
  }, [currentPageRows]);
  const handleColumnHeaderKeyDown = React101.useCallback((params, event) => {
  ...
  const handleHeaderFilterKeyDown = React101.useCallback((params, event) => {
    const dimensions = apiRef.current.getRootDimensions();
    if (!dimensions) {
      return;
    }
  ...
  const handleColumnGroupHeaderKeyDown = React101.useCallback((params, event) => {
    const dimensions = apiRef.current.getRootDimensions();
    if (!dimensions) {
      return;
    }
  ...
  const checkIfCanStartEditing = React101.useCallback((initialValue, {
    event
  }) => {
    if (event.key === " ") {
      return false;
...
var useGridPagination = (apiRef, props) => {
  ...
  const setPage = React102.useCallback((page) => {
  ...
  const setPageSize = React102.useCallback((pageSize) => {
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const setPaginationModel = React102.useCallback((paginationModel) => {
    var _props$rowCount;
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const handleUpdateAutoPageSize = React102.useCallback(() => {
    const dimensions = apiRef.current.getRootDimensions();
...
var useGridPreferencesPanel = (apiRef, props) => {
  ...
  const hideTimeout = React103.useRef();
  const immediateTimeout = React103.useRef();
  const hidePreferences = React103.useCallback(() => {
  ...
  const doNotHidePanel = React103.useCallback(() => {
    immediateTimeout.current = setTimeout(() => clearTimeout(hideTimeout.current), 0);
  }, []);
  const hidePreferencesDelayed = React103.useCallback(() => {
    hideTimeout.current = setTimeout(hidePreferences, 100);
  }, [hidePreferences]);
  const showPreferences = React103.useCallback((newValue, panelId, labelId) => {
...
var useGridCellEditing = (apiRef, props) => {
  ...
  const handleCellEditStart = React104.useCallback((params) => {
    ...
    if (reason === GridCellEditStartReasons.printableKeyDown) {
      if (React104.version.startsWith("17")) {
...
var useGridRows = (apiRef, props) => {
  ...
  const throttledRowsChange = React107.useCallback(({
    cache,
    throttle
  }) => {
    const run = () => {
  ...
  const getAllRowIds = React107.useCallback(() => gridDataRowIdsSelector(apiRef), [apiRef]);
...
var useGridRowSelection = (apiRef, props) => {
  ...
  const propRowSelectionModel = React109.useMemo(() => {
  ...
  const setRowSelectionModel = React109.useCallback((model) => {
  ...
  const isRowSelected = React109.useCallback((id) => gridRowSelectionStateSelector(apiRef.current.state).includes(id), [apiRef]);
  ...
  const handleRowClick = React109.useCallback((params, event) => {
    var _closest;
...
var useGridRowSelectionPreProcessors = (apiRef, props) => {
  ...
  const updateSelectionColumn = React110.useCallback((columnsState) => {
...
var useGridSorting = (apiRef, props) => {
  ...
  const upsertSortModel = React111.useCallback((field, sortItem) => {
  ...
  const createSortItem = React111.useCallback((col, directionOverride) => {
  ...
  const addColumnMenuItem = React111.useCallback((columnMenuItems, colDef) => {
  ...
  const applySorting = React111.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const setSortModel = React111.useCallback((model) => {
  ...
  const sortColumn = React111.useCallback((column, direction, allowMultipleSorting) => {
    if (!column.sortable) {
      return;
    }
  ...
  const getSortModel = React111.useCallback(() => gridSortModelSelector(apiRef), [apiRef]);
  const getSortedRows = React111.useCallback(() => {
    const sortedRows = gridSortedRowEntriesSelector(apiRef);
    return sortedRows.map((row) => row.model);
  }, [apiRef]);
  const getSortedRowIds = React111.useCallback(() => gridSortedRowIdsSelector(apiRef), [apiRef]);
  const getRowIdFromRowIndex = React111.useCallback((index) => apiRef.current.getSortedRowIds()[index], [apiRef]);
  const sortApi = {
    getSortModel,
    getSortedRows,
    getSortedRowIds,
  ...
  const stateExportPreProcessing = React111.useCallback((prevState, context) => {
    var _props$initialState2, _props$initialState2$;
  ...
  const stateRestorePreProcessing = React111.useCallback((params, context) => {
    var _context$stateToResto;
  ...
  const flatSortingMethod = React111.useCallback((params) => {
    const rowTree = gridRowTreeSelector(apiRef);
  ...
  const handleColumnHeaderClick = React111.useCallback(({
    colDef
  }, event) => {
    const allowMultipleSorting = event.shiftKey || event.metaKey || event.ctrlKey;
    sortColumn(colDef, void 0, allowMultipleSorting);
  }, [sortColumn]);
  const handleColumnHeaderKeyDown = React111.useCallback(({
    colDef
  }, event) => {
  ...
  const handleColumnsChange = React111.useCallback(() => {
    const sortModel = gridSortModelSelector(apiRef);
  ...
  const handleStrategyProcessorChange = React111.useCallback((methodName) => {
...
var useGridScroll = (apiRef, props) => {
  ...
  const scrollToIndexes = React112.useCallback((params) => {
  ...
  const scroll = React112.useCallback((params) => {
  ...
  const getScrollPosition = React112.useCallback(() => {
...
function useGridDimensions(apiRef, props) {
  ...
  const errorShown = React113.useRef(false);
  const rootDimensionsRef = React113.useRef(null);
  const fullDimensionsRef = React113.useRef(null);
  const rowsMeta = useGridSelector(apiRef, gridRowsMetaSelector);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const rowHeight = Math.floor(props.rowHeight * densityFactor);
  const totalHeaderHeight = getTotalHeaderHeight(apiRef, props.columnHeaderHeight);
  const updateGridDimensionsRef = React113.useCallback(() => {
  ...
  const [savedSize, setSavedSize] = React113.useState();
  const debouncedSetSavedSize = React113.useMemo(() => debounce(setSavedSize, 60), []);
  const previousSize = React113.useRef();
  useEnhancedEffect_default(() => {
    if (savedSize) {
  ...
  const resize = React113.useCallback(() => {
    apiRef.current.computeSizeAndPublishResizeEvent();
  }, [apiRef]);
  const getRootDimensions = React113.useCallback(() => fullDimensionsRef.current, []);
  const getViewportPageSize = React113.useCallback(() => {
    const dimensions = apiRef.current.getRootDimensions();
    if (!dimensions) {
      return 0;
    }
  ...
  const computeSizeAndPublishResizeEvent = React113.useCallback(() => {
  ...
  const isFirstSizing = React113.useRef(true);
  const handleResize = React113.useCallback((size) => {
...
var useGridVirtualScroller = (props) => {
  ...
  const currentPage = useGridVisibleRows(apiRef, rootProps);
  const renderZoneRef = React114.useRef(null);
  const rootRef = React114.useRef(null);
  const handleRef = useForkRef(ref, rootRef);
  const [renderContext, setRenderContext] = React114.useState(null);
  const prevRenderContext = React114.useRef(renderContext);
  const scrollPosition = React114.useRef({
    top: 0,
    left: 0
  });
  const [containerDimensions, setContainerDimensions] = React114.useState({
    width: null,
    height: null
  });
  const prevTotalWidth = React114.useRef(columnsTotalWidth);
  const rowStyleCache = React114.useRef(/* @__PURE__ */ Object.create(null));
  const prevGetRowProps = React114.useRef();
  const prevRootRowStyle = React114.useRef();
  const getRenderedColumnsRef = React114.useRef(defaultMemoize((columns, firstColumnToRender, lastColumnToRender) => {
    return columns.slice(firstColumnToRender, lastColumnToRender);
  }));
  const getNearestIndexToRender = React114.useCallback((offset) => {
  ...
  const computeRenderContext = React114.useCallback(() => {
  ...
  const handleResize = React114.useCallback(() => {
    if (rootRef.current) {
      setContainerDimensions({
        width: rootRef.current.clientWidth,
        height: rootRef.current.clientHeight
  ...
  const updateRenderZonePosition = React114.useCallback((nextRenderContext) => {
    const [firstRowToRender, lastRowToRender] = getRenderableIndexes({
  ...
  const updateRenderContext = React114.useCallback((nextRenderContext) => {
  ...
  const contentSize = React114.useMemo(() => {
  ...
  const getRenderContext = React114.useCallback(() => {
...
function GridColumnGroupHeader(props) {
  ...
  const rootProps = useGridRootProps();
  const headerCellRef = React115.useRef(null);
  const apiRef = useGridApiContext();
  ...
  const renderParams = React115.useMemo(() => ({
    groupId,
    headerName,
    description,
    depth,
  ...
  const publish = React115.useCallback(
    (eventName) => (event) => {
      if (!event.currentTarget.contains(event.target)) {
        return;
      }
  ...
  const mouseEventsHandlers = React115.useMemo(() => ({
...
var GridBaseColumnHeaders = React117.forwardRef(function GridColumnHeaders(props, ref) {
  const {
    className
...
var GridColumnHeaders2 = React119.forwardRef(function GridColumnsHeaders(props, ref) {
  const {
    innerRef,
    visibleColumns,
    sortColumnLookup,
...
var GridNoResultsOverlay = React120.forwardRef(function GridNoResultsOverlay2(props, ref) {
  const apiRef = useGridApiContext();
...
var GridColumnUnsortedIcon = React121.memo(function GridColumnHeaderSortIcon2(props) {
  const {
    sortingOrder
...
var useGridRowsMeta = (apiRef, props) => {
  ...
  const rowsHeightLookup = React125.useRef(/* @__PURE__ */ Object.create(null));
